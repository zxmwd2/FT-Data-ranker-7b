...
TOKEN_NUMS = 10000000
RATIO = 0.8
EN_DATA_DIR = "/workspace/data/en.jsonl"
ZH_DATA_DIR = "/workspace/data/zh.jsonl"
OUTPUT_FILES = "/workspace/data/7b1203.jsonl"
...
def findAllFile(base):
    ...
    return files
def filter_en(example):
  return len(example["output"])>384
def filter_zh(example):
  return len(example["output"])>124

enc = AutoTokenizer.from_pretrained("/workspace/data/models/Baichuan2-7B-Base", use_fast=False, trust_remote_code=True)
...
ds_en = load_dataset('json', data_files=en_files, split='train').filter(filter_en)
...
if ZH_DATA_DIR:
    ...
    ds_zh = load_dataset('json', data_files=zh_files, split='train').filter(filter_zh)
    count = 0
